...
from apiTest.apiRecordExecution.models import ApiTaskRunRecord
...
router = APIRouter(tags=['API测试计划'], dependencies=[Depends(is_authenticated)])


# 创建测试计划
@router.post("/task", summary="创建API任务", response_model=TaskSchemas, status_code=status.HTTP_201_CREATED)
async def create_task(item: AddTaskForm):
    try:
        ...
        if task_type == 1:
            task = await ApiTask.create(**item.model_dump())
        else:
...
# 获取测试计划列表
@router.get("/task", summary="API任务列表", status_code=status.HTTP_200_OK)
async def get_task(page: int = 1, size: int = 10, search=None, task_type: int = None, project_id: int = None):
    ...
    # 分别构建UI和API任务的查询
    api_query = ApiTask.filter(project_id=project_id).prefetch_related("suites")

    # 添加搜索条件
    if search:
        api_query = api_query.filter(name__icontains=search)

    total_count = await api_query.count()
    paginated_tasks = await api_query.offset((page - 1) * size).limit(size).order_by("-create_time")

    result = []
    # 处理所有任务
    for task in paginated_tasks:
        run_record = await ApiTaskRunRecord.filter(task=task.id).order_by("-id").first()
        run_count = await ApiTaskRunRecord.filter(task=task.id).count()
...
# 删除测试计划
@router.delete("/task/{task_id}/{task_type}", summary="删除API任务", status_code=status.HTTP_204_NO_CONTENT)
async def delete_task(task_id: int, task_type: int = None):
    try:
        if task_type == 1:
            task = await ApiTask.get_or_none(id=task_id)
        else:
...
# 修改测试计划名称
@router.put("/task/{task_id}", summary="修改API任务", response_model=TaskSchemas, status_code=status.HTTP_200_OK)
...
# 往任务中添加业务流试(套件)
@router.post("/task/{task_id}/suite", tags=['测试计划'], summary="API任务中添加套件",
             status_code=status.HTTP_201_CREATED, response_model=TaskSchemas)
...
# 删除任务中的业务流(套件)
@router.delete("/task/{task_id}/suite/{suite_id}/{task_type}", summary="删除API任务中套件",
               status_code=status.HTTP_204_NO_CONTENT)
async def delete_step(task_id: int, suite_id: int, task_type: int = None):
    try:
        if task_type == 1:
            task = await ApiTask.get_or_none(id=task_id)
            suite = await ApiTestSuite.get_or_none(id=suite_id)
        else:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="传入的任务类型错误")
...
# 获取测试计划详情(任务中所有的套件)
@router.get("/task/{task_id}", summary="API任务详情", response_model=TaskDetailSchemas, status_code=status.HTTP_200_OK)
async def get_task_detail(task_id: int, task_type: int = None):
    try:
        if task_type == 1:
            task = await ApiTask.get_or_none(id=task_id).prefetch_related("suites")
        else:
            raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="传入的任务类型错误")
        if not task:
